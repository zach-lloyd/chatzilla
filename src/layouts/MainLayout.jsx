...
function MainLayout() {
  const { BASE_URL, user, setUser, setIsAuthenticated } =
  ...
  return (
    <div className="container-fluid d-flex flex-column p-0 m-0 min-vh-100">
      <header className="p-0 d-flex justify-content-between align-items-center">
        <NavPanel />
        {/* 
            Check for user information and either display the username or an error
            message indicating information is missing 
        */}
        <div className="p-2">
          ...
          <button onClick={handleSignOut} className="btn btn-monster btn-sm">
            Log Out
          </button>
        </div>
      </header>
      ...
      <main className="flex-grow-1 container-fluid p-3">
        {/* 
            The Outlet renders the matched child route (HomePage, ProfilePage, 
            etc.). 
        */}
        <Outlet />
      </main>
