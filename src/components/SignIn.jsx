...
function SignIn() {
  ...
  const handleSignIn = async () => {
    ...
    const response = await fetch(`${BASE_URL}/users/sign_in`, {
      ...
      headers: {
        ...
        "X-CSRF-Token": csrfToken, // Required by Rails for non-GET requests.
      },
    ...
    if (response.ok) {
      // If the authentication was successful, store the user's data and update
      // auth status.
  ...
  return (
    <div className="card p-4 shadow-sm w-100 h-sm-100 w-md-75 mt-5">
      ...
      <button onClick={handleSignIn} className="btn btn-monster w-100">
        Sign In
      </button>

      <div className="d-flex gap-2">
        <p>First time user?</p>
        <Link className="text-custom-green" to="/sign_up">
          Sign Up
        </Link>
      </div>
    </div>
